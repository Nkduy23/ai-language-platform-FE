  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-2">Blog học ngoại ngữ</h1>
        <p className="text-sm sm:text-base text-slate-500 mb-8 sm:mb-10">Mẹo học tiếng Anh, Trung, Nhật hiệu quả cùng AI</p>

        {posts.length === 0 ? (
          <p className="text-slate-400 text-sm sm:text-base">Chưa có bài viết nào. Quay lại sau nhé!</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            {posts.map((post) => (
              <Link key={post.id} href={`/marketing/blog/${post.slug}`} className="block rounded-xl border border-surface-border p-4 sm:p-5 hover:shadow-card transition-shadow">
                <span className="text-xs text-brand font-medium">{post.language}</span>
                <h2 className="font-semibold text-slate-900 mt-1 mb-2 text-sm sm:text-base">{post.title}</h2>
