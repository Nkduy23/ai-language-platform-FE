            ...
            {!data || data.data.length === 0 ? (
              ...
              data.data.map((n) => (
                <Link
                  ...
                  className={cn(
                    "block px-4 py-3 border-b border-surface-border last:border-0 hover:bg-slate-50",
                    !n.isRead && "bg-brand/5",
                  )}
                >
                  <p className="text-sm font-medium text-slate-800">{n.title}</p>
                  {n.body && <p className="text-xs text-slate-500 mt-0.5 line-clamp-2">{n.body}</p>}
                  <p className="text-xs text-slate-400 mt-1">{new Date(n.createdAt).toLocaleDateString("vi-VN")}</p>
                </Link>
              ))
            )}
          </div>
