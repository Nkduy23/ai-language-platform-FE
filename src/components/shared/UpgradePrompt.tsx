    ...
    <Modal open={open} onClose={onClose} title="Nâng cấp gói" size="lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
        {paidPlans.map((p) => {
          ...
          const accent = key === "premium" ? "border-gold-foil bg-gold-foil/10" : "border-airmail bg-airmail/5";
          return (
            <button key={p.plan} onClick={() => setSelectedPlan(key)} className={cn("text-left rounded-md border-[1.5px] p-4 transition-colors", isSelected ? accent : "border-surface-border")}>
              <p className="font-display font-semibold text-ink-navy">{p.plan}</p>
              <p className="text-2xl font-bold text-airmail my-1">
                {p.priceVnd.toLocaleString("vi-VN")}đ<span className="text-sm text-ink-muted font-normal">/tháng</span>
              </p>
              <ul className="space-y-1 mt-2">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-1.5 text-xs text-ink-muted">
                    <Check className="w-3.5 h-3.5 text-stamp-teal shrink-0 mt-0.5" /> {f}
                  </li>
                ))}
              </ul>
            </button>
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-5">
        <button
          ...
          className={cn("flex-1 rounded-md border-[1.5px] px-3 py-2 text-sm", gateway === "stripe" ? "border-airmail bg-airmail/5 text-airmail" : "border-surface-border text-ink-muted")}
        >
          💳 Thẻ quốc tế (Stripe)
        </button>
        <button
          onClick={() => setGateway("vnpay")}
          className={cn("flex-1 rounded-md border-[1.5px] px-3 py-2 text-sm", gateway === "vnpay" ? "border-airmail bg-airmail/5 text-airmail" : "border-surface-border text-ink-muted")}
        >
          🏦 VNPay (ATM/QR)
        </button>
      </div>
